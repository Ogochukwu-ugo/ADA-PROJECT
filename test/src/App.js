...
function App() {
  ...
  const [people, updatePeople, peopleDetail] =useState([
  ...
  const openDetail = (id) =>{
    let detail = people.find(each => each.id === id)
    peopleDetail(detail)
  }
  ...
  return (
    <div  style={{ color: 'blue' }} className="App">
     <div className="container">
      <div className="row">
      {people.map(item =>{
        return(<Card key={item.id} name={item.name} profilePics={item.profilePics} id={item.id} description={item.description} openDetail={openDetail}  handleDelete={handleDelete} />)
      })}
      </div>
     </div>
     {/* <Modal /> */}
